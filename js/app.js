...
(function () {
  ...
  function renderAccountSwitch() {
    const options = state.accounts.map(a => `<option value="${a.id}">${a.name}</option>`).join('');
    const sel = $('#account-switch');
    sel.innerHTML = options;
    sel.value = state.activeAccountId;
    const selMobile = $('#account-switch-mobile');
    if (selMobile) { selMobile.innerHTML = options; selMobile.value = state.activeAccountId; }
    const acc = activeAccount();
  ...
  $('#account-switch').addEventListener('change', (e) => onAccountChange(e.target.value));
  ...
  function registerServiceWorker() {
    ...
    navigator.serviceWorker.register('sw.js').then((reg) => {
      // Vérifie activement une nouvelle version chaque fois que l'app revient au premier plan
      document.addEventListener('visibilitychange', () => {
        if (document.visibilityState === 'visible') reg.update().catch(() => {});
      });
      window.addEventListener('focus', () => reg.update().catch(() => {}));
      // Vérification immédiate au chargement, au cas où une version plus récente existe déjà
      reg.update().catch(() => {});
    ...
    // pour afficher automatiquement la mise à jour, sans action de l'utilisateur.
